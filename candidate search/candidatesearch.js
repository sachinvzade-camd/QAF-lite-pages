...
function getObjectID() {
 fieldListObject = []
  window.QafService.GetObjectById('Candidate').then((responses) => {
...
function addInTracker(){
  ...
  window.QafService.GetItems(objectName, fieldList, pageSize, pageNumber, whereClause, '', orderBy).then((candidate) => {
    if (Array.isArray(candidate) && candidate.length > 0) {
      ...
      let jobpost=jobpostlist.find(job=>job.RecordID===jobPostid)
       object = {
        ...
        JobPost: jobpost.RecordID + ";#" + jobpost.JobID + "-" + jobpost.JobTitle,
        ...
        Resume: shortListcandidate.AttachResume,
        ExpectedSalary: shortListcandidate.Expectedsalary,
        Vendor: shortListcandidate.AgencyName,
        Recruiter: JSON.stringify(recruiter)
      }
      let excludeFormField = ["YearsofExperience", "AttachResume", "Presentsalary", "Mobile", "Expectedsalary", "AgencyName", "Recruiter"]
      let newFieldsTracker = fieldListObject.filter((objOne) => {
        return !excludeFormField.some((objTwo) => {
          return objOne === objTwo;
        });
      });
      newFieldsTracker.forEach(obj => {
        object[obj] = shortListcandidate[obj]
      })
...
async function iscandidateshortlisted() {
  let selectElement = document.getElementById("interviewer");
  var selectedOption = selectElement.options[selectElement.selectedIndex];
  var CandidateId = selectedOption.getAttribute("data-value1");
  let objectName = "Job_Tracker";
  let list = "Candidate,JobPost";
  let orderBy = "";
  let whereClause =`Candidate='${CandidateId}'`; 
  let fieldList = list.split(";#")
  let pageSize = "20000";
  let pageNumber = "1";
  window.QafService.GetItems(objectName, fieldList, pageSize, pageNumber, whereClause, '', orderBy).then(shortedlist=>{
    if (Array.isArray(shortedlist) && shortedlist.length > 0) {
      alert("candidate already in tracker")
    }else{
      addInTracker()
    }
  })
...
function save(object, repositoryName) {
  return new Promise((resolve) => {
    ...
    alert("Candidate added in tracker")
     getcandidateList()
      resolve({
        response
      })
...
function whatsAppClick(row) {
  ...
  if(Mobile){
    let link = `https://wa.me/${Mobile}?text=${this.encodeForWhatUp(message)}`
  ...
  window.QafService.GetItems(objectName, fieldList, pageSize, pageNumber, whereClause, '', orderBy).then((candidateList) => {
    if (Array.isArray(candidateList) && candidateList.length > 0) {
      ...
      <div class="main">
          ...
          <div class="row2">
            <div class="column">
              ...
              <div class="experiance">
                <p>â‚¹&nbsp;${candidate.Expectedsalary == null ? "" : candidate.Expectedsalary}&nbsp;|&nbsp;${candidate.YearsofExperience == null ? "" : candidate.YearsofExperience}Y&nbsp;|&nbsp; ${candidate.NoticePeriod == null ? "" : candidate.NoticePeriod}M &nbsp;|&nbsp; ${candidate.CurrentLocation == null ? "" : candidate.CurrentLocation}</p>
              </div>
            </div>
            <div class="contact">
              <div class="number">
        ...
        <div class="job">
          <p><em>Current Salary:</em>&nbsp; <span>â‚¹&nbsp;${candidate.Presentsalary == null ? "" : candidate.Presentsalary}</span></p>
