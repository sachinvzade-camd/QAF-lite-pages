...
function getJobPosting() {
    ...
    let list = "JobTitle,RecordID,JobImage,PostingExpiresOn,IsFresher";
    let orderBy = "";
    // let whereClause="";
    let whereClause = "(PostingExpiresOn>='" +startDate + "'<OR>PostingExpiresOn='')<<NG>>(IsFresher='true')";
    let fieldList = list.split(",")
    let pageSize = "20000";
    let pageNumber = "1";
        ...
        if (Array.isArray(jobsList) && jobsList.length > 0) {
          ...
          let carttemplate="";
            jobsList.forEach(val => {
                ...
                carttemplate += `
                <div class="card" onclick="openJobDetails('${val.RecordID}')">
