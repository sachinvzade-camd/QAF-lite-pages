...
// tableHead += '<th class="action-head qaf-th">Actions</th>';
// TableColumns.forEach(column => {
//     tableHead += `<th class="qaf-th">${column.DN}<i class="fa fa-sort sorted-button" aria-hidden="true" onclick="sortTable('${column.INN}')"></i></th>`;
// });

// tableRow = '<div class="nodata">No Record Found</div>';
// let nodata = `
// <thead class="qaf-thead">
//     <tr class="qaf-tr">
//         ${tableHead}
    ...
    fetch(`${apURL}/api/SalaryHeads?employeeID=${selectedEmployee}&ctc=0`, {
        ...
        .then(data => {
            if (Array.isArray(data) && data.length > 0) {
             earning = data.filter(a => a.Category.toLowerCase()==="Earning".toLowerCase())
...
EmployeeName.addEventListener('change', handleMakeNextButtonDisable);
...
function onFileChangeFile(){
  var fileToRead = document.getElementById("upload");
    fileToRead.click();
    fileToRead.onchange = () => {
...
function downloadTemplate() {
    let csvBody="";
    let csvHeader = ["Employee ID","Basic","HRA","Fixed Allowance","StatutoryBonus","ProfessionalTax","PF-EmployeeContribution","VPFEmployeeContribution","ESI-Employeecontribution","Income Tax","ESI-Employercontribution","PF - Employer Contribution","Gratuity","Food Coupon","Variable","Petrol Reimbursement","TDS","Medical Allowance","Conveyance Allowance","Arrears","Salary Advance","MLWF","Other Deduction"];
    csvHeader = csvHeader.join(',') + '\n';
    var hiddenElement = document.createElement('a');
    hiddenElement.href = 'data:text/csv;charset=utf-8,' + encodeURI(csvHeader + csvBody);
    hiddenElement.target = '_blank';
