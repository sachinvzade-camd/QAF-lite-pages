    ...
    for (let hour = 0; hour < 24; hour++) {
        ...
        if (EventForSameTime.length > 0) {
            ...
            EventForSameTime.forEach(event => {
                ...
                const duration = parseInt(ToTime.split(':')[0]) - parseInt(FromTime.split(':')[0]) + 1;

                eventCell.setAttribute('rowspan', duration);
...
function AddForm() {
    // loadTable()
    console.log("AllMeetingRequestData", Meeting_Room_Request)
...
function SaveRecord() {
    ...
    console.log("meetingroomValue ", meetingroomValue);
    console.log("fromTime ", NewfromTime);

    if (RequestFor == "") {
        alert("Request For field is required");
    }
    else if (requesttitle.trim() == "") {
        alert("Brief about the request field is required");
    }
    else if (requesttitle.trim().length > 20) {
        alert("Brief about the request allow 20 characters");
    }
    else if (meetingroom == "") {
        alert("Meeting Room field is required");
    }
    else if (fromDate == "") {
        alert("From Time field is required");
    }
    else if (fromtime == "") {
        alert("From Time field is required");
    }
        ...
        alert("To Time field is required");
    }
    else {
        console.log("AllMeetingRequestData", Meeting_Room_Request)
        const Meeting_FromDate = moment(NewfromTime)
        const Meeting_ToDate = moment(NewtoTime)
        const meetingsWithinRange = [];
        let Ismeetingdataloading = true;
        let newState=false
        AllMeetingRequestData.forEach(request => {
            const requestFromTime = moment(adjustTimeToIST(request.FromTime)).format("YYYY-MM-DD HH:mm:ss A");
            const requestToTime = moment(adjustTimeToIST(request.ToTime)).format("YYYY-MM-DD HH:mm:ss A");

            const form_field_FromDate = (Meeting_FromDate).format("YYYY-MM-DD");
            const form_field_FromTime = (Meeting_FromDate).format("hh:mm:ss");
            const combinefromDate = moment(`${form_field_FromDate} ${form_field_FromTime}`).format("YYYY-MM-DD HH:mm:ss A");

            const form_field_toDate = (Meeting_ToDate).format("YYYY-MM-DD");
            const form_field_toTime = (Meeting_ToDate).format("hh:mm:ss");
            const combineToDate = moment(`${form_field_toDate} ${form_field_toTime}`).format("YYYY-MM-DD HH:mm:ss A");
            ...
            // var isBetweenDates1 = moment(requestFromTime).isBetween(combinefromDate, combineToDate, 'date', '[]');
            // var isBetweenDates2 = moment(requestToTime).isBetween(combinefromDate, combineToDate, 'date', '[]');
            ...
            var isBetweenDates2 = moment(combineToDate).isBetween(requestFromTime, requestToTime, 'date', '[]');
                ...
                if (isBetweenDates1 && isBetweenDates2) {
                    ...
                    // let firstTime=moment(requestFromTime,timeFormat).isBetween(moment(combinefromDate, timeFormat), moment(combineToDate, timeFormat)) ||
                    // moment(requestFromTime,timeFormat).isSame(moment(combinefromDate, timeFormat)) ||
                    // moment(requestFromTime,timeFormat).isSame(moment(combineToDate, timeFormat))

                    // let secondTime=moment(requestToTime,timeFormat).isBetween(moment(combinefromDate, timeFormat), moment(combineToDate, timeFormat)) ||
                    // moment(requestToTime,timeFormat).isSame(moment(combinefromDate, timeFormat)) ||
                    // moment(requestToTime,timeFormat).isSame(moment(combineToDate, timeFormat))
                    ...
                    // let firstTime=moment(time1).isBetween(moment(req1), moment(req2)) ||
                    // moment(time1).isSame(moment(req1)) ||
                    // moment(time1).isSame(moment(req2))

                    // let secondTime=moment(time2).isBetween(moment(req1), moment(req2)) ||
                    // moment(time2).isSame(moment(req1)) ||
                    // moment(time2).isSame(moment(req2))
                    let firstTime =isTimeBetween(req1,req2,time1)
                    let secondTime =isTimeBetween(req1,req2,time2)

                    if(firstTime||secondTime){
                        Ismeetingdataloading=false;
                        meetingsWithinRange.push(request)
                    }
        ...
        if (meetingsWithinRange.length > 0) {
            alert("Selected Meeting room and time slot already booked.")
        }
        else {
...
const Data = [
    {
        ...
        "MeetingRoom": "02831e9e-0490-455d-9c65-6e53ee57c321;#Creative Space",
        "FromTime": "2024-05-02T03:15:00",
        "ToTime": "2024-05-02T04:15:00",
        "IsApproved": true,
