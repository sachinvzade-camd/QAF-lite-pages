...
var employeeList;
 let expenseGridReport = {
    ...
    columns: [
          ...
          {field:'TotalAmount', displayName:'Total Claimed  Amount', sorting: true},
          {field:'Description', displayName:'Description', sorting: true},
    ],
    viewFields: ['Requesttitle','Approvedamount','Department','TotalAmount','Description'],
    page: 1,
    pageSize: 10,
...
let gridExpenseColumnsReport = [
    ...
    {field:'Department', displayName:'Department',sequence:2, sorting: true},
    {field:'TotalAmount', displayName:'Total Claimed  Amount',sequence:4, sorting: true},
      ...
      window.QafService.GetItems(expenseGridReport.repository, expenseGridReport.viewFields, expenseGridReport.pageSize, expenseGridReport.page, filterGridCondition,'',orderBy).then((filteredExpense)=>{
        if(filteredExpense&&filteredExpense.length>0){
          filteredExpense.forEach(val=>{
            ...
            val.Approvedamount=val.Approvedamount||val.Approvedamount===0?val.Approvedamount:''
            val.TotalAmount=val.TotalAmount||val.TotalAmount===0?val.TotalAmount:''
...
let orderBy="false"
        ...
        window.QafService.GetItems(expenseGridReport.repository, expenseGridReport.viewFields, expenseGridReport.pageSize, 1, filterGridCondition, page.detail.field, page.detail.order,'',orderBy).then((filteredExpense)=>{
          if(filteredExpense&&filteredExpense.length>0){
            filteredExpense.forEach(val=>{
              val.Department=formatGridValue(val.Department);
              val.Approvedamount=val.Approvedamount||val.Approvedamount===0?val.Approvedamount:''
